...
- 首要用户仍是项目所有者本人，同时正式面向没有 Python/Node.js 开发环境的普通 Windows 用户提供便携发行形态；当前运行边界保持本机、单用户、一次打开一份文档。
...
- 普通用户可以下载 Windows 便携发行包、解压并双击启动；程序使用自带隔离运行时，程序自有配置、模型、字体、缓存、临时文件和日志由用户在解压目录内统一掌控。
...
10. **便携隔离且不破坏开发模式。** 面向普通用户的发行版不依赖、不修改系统 Python，不在运行或翻译时安装 Python 包；程序自有可变数据只写入便携目录，不能静默散落到用户或系统环境。发行层复用唯一业务源码并与构建产物隔离，不能改写仓库开发 `venv`、配置、缓存、日志或取消现有开发启动方式。

## 产品边界

当前产品明确服务本机、单用户、单文档阅读，并以 Windows 10/11 x64 便携 ZIP 作为首个普通用户发行目标。便携发行实施边界与验收顺序由 [Windows 便携发行实施清单](<improvement items/windows-portable-release-plan.md>) 维护。它不是现成的多用户 Web 服务、云端文档平台、通用知识库或完整 PDF 编辑器。
