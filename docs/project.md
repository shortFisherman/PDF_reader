...
当前产品明确服务本机、单用户、单文档阅读，并以 Windows 10/11 x64 便携 ZIP 作为首个普通用户发行目标。便携发行实施边界与验收顺序由 [Windows 便携发行实施清单](windows-portable-release-plan.md) 维护。它不是现成的多用户 Web 服务、云端文档平台、通用知识库或完整 PDF 编辑器。
