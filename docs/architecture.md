...
| `tests/` | 36 个 pytest 文件（含 P2-07 路径、P2-06 任务日志、P2-01 包布局、P2-03 契约、P3-04 许可证与 P3-06 上游契约/文档治理用例）与前端 `.mjs` 测试运行器 |
...
错误传播：`TranslationError` 与普通异常都被 `generate`/`generate_batch` 捕获并输出 SSE `error` 事件，同时记录上下文日志；任务工作区（根目录，含 `input/` 与 `output/`）只有在 worker 线程确认退出后才会在 `finally` 中整体清理（join timeout 时整体保留并记 WARNING）。
