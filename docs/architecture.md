...
| `packaging/windows/build.ps1`、`*.spec`、`buildtool/` | P2-01 Windows 原生 onedir 构建层：在 `build/release-windows/` 创建独立 Python 3.12 venv，从锁定运行时/构建工具环境驱动启动器与服务双 spec，显式收集资源、动态导入、包数据与原生 DLL，在 `dist/release-windows/` 组装唯一顶层启动入口并生成 runtime/release manifest、文件清单、ZIP 与 SHA-256；所有写路径、清理根和链接边界由可测试的标准库 buildtool 验证，失败时 fail closed，不触碰开发 `venv/`、`config.toml`、`cache/`、`logs/` |
...
9. **P2-01 冻结构建层已建立，真实冻结产物仍待 Windows 本地构建。** 便携服务在 `sys.frozen` 时会在第三方导入前验证自身路径、用户 site 禁用状态与 `sys.path` 的 app 内边界；运行时锁、双 PyInstaller onedir spec、资源/动态导入/原生 DLL 收集、源码与最终产物策略门、manifest/文件清单/ZIP/SHA-256 生成以及安全清理契约均已固定。PyInstaller 不能在 Linux 交叉生成 Windows 产物；Windows Python 直接访问 `\\wsl.localhost` 又存在 UNC/WSL 文件语义差异，因此真实构建必须在 Windows 本地 NTFS checkout 执行。当前尚无已通过策略门的最终目录和 ZIP，故最终 DLL/动态导入完整性、真实 wheel SHA-256 manifest、隐藏系统 Python 后启动与翻译及 Process Monitor 审计仍不能视为通过；前两项完成 P2-01，后两项属于 P2-02。
