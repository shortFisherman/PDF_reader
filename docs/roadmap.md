...
| 方向 | 状态 | 授权与执行台账 |
|---|---|---|
| Windows 10/11 x64 便携发行版：解压后双击启动，自带隔离 Python 运行时，程序自有配置、模型、字体、缓存、临时文件与日志只写入 EXE 所在目录 | `已授权（授权见链接的独立变更）` | [Windows 便携发行实施清单](windows-portable-release-plan.md)；用户于 2026-09-04 明确确认方向并授权进入执行阶段 |
...
| 日期 | 决定 | 状态 | 依据 |
|---|---|---|---|
| 2026-09-04 | 首个普通用户发行形态采用 Windows x64 便携 ZIP；发行版自带隔离 Python 运行时、禁止运行时安装 Python 包，所有程序自有可变数据保存在解压目录内 | `已授权实施` | [Windows 便携发行实施清单](windows-portable-release-plan.md) |
