...
早期识别的基础可靠性候选（不可变文档身份、服务端任务互斥与生命周期、SSE 断开和后台任务所有权、依赖锁可移植安装、术语表与 PDF 替换事务）已作为 [工程与架构长期改进清单](engineering-improvement-plan-829.md) 的 P0/P1 条目实施并验收；工程卫生与生命周期事项（P3-01 至 P3-07）也已于 2026-08-30 全部收口。清单中 P0 至 P3 全部条目均已收口并记录提交与验证证据，不再列为路线图候选；收口只表示既有工程项结束，不改变本文档候选方向仍未授权实施的开放状态。本文件继续保存产品方向与体验类候选。
