...
工程改进清单 P0 至 P3 已全部收口（2026-08-30，最终实现树 `1ca789d`），收口本身不等于授权任何新的产品方向；本文档的开放问题仍保持未授权候选状态。

## 基础可靠性工作状态

早期识别的基础可靠性候选（不可变文档身份、服务端任务互斥与生命周期、SSE 断开和后台任务所有权、依赖锁可移植安装、术语表与 PDF 替换事务）已作为 [工程与架构长期改进清单](completed%20improvements/engineering-improvement-plan-829.md) 的 P0/P1 条目实施并验收；工程卫生与生命周期事项（P3-01 至 P3-07）也已于 2026-08-30 全部收口。清单中 P0 至 P3 全部条目均已收口并记录提交与验证证据，不再列为路线图候选；收口只表示既有工程项结束，不改变本文档候选方向仍未授权实施的开放状态。本文件继续保存产品方向与体验类候选。
...
| 方向 | 状态 | 授权与执行台账 |
|---|---|---|
| Windows 10/11 x64 便携发行版：解压后双击启动，自带隔离 Python 运行时，程序自有配置、模型、字体、缓存、临时文件与日志只写入 EXE 所在目录 | `已授权（授权见链接的独立变更）` | [Windows 便携发行实施清单](<improvement items/windows-portable-release-plan.md>)；用户于 2026-09-04 明确确认方向并授权进入执行阶段 |
...
| 日期 | 决定 | 状态 | 依据 |
|---|---|---|---|
| 2026-09-04 | 首个普通用户发行形态采用 Windows x64 便携 ZIP；发行版自带隔离 Python 运行时、禁止运行时安装 Python 包，所有程序自有可变数据保存在解压目录内 | `已授权实施` | [Windows 便携发行实施清单](<improvement items/windows-portable-release-plan.md>) |
