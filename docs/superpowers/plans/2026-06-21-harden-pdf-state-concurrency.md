...
- [x] **Step 1: 在 `tests/test_state.py` 顶部增加 import**
...
- [x] **Step 2: 在 `tests/test_state.py` 末尾追加渲染竞态测试**
...
- [x] **Step 3: 在 `tests/test_state.py` 末尾追加并发 replace 不同页测试**
...
- [x] **Step 4: 在 `tests/test_state.py` 末尾追加慢 IO 不阻塞测试**
...
- [x] **Step 5: 在 `tests/test_routes.py` 末尾追加越界页码测试**
...
- [x] **Step 1: 修改 `state.py` 的 `render_page` 方法**
...
- [x] **Step 1: 在 `AppState.__init__` 新增 `_write_lock`**
...
- [x] **Step 2: 重构 `state.py` 的 `replace_page` 方法**
...
- [x] **Step 1: 在 `routes.py` 的 `translate_page` 增加页码校验**
...
- [x] **Step 3: 确认 requirements.lock 无变更**

Run: `git diff --name-only`
Expected: 无 `requirements.lock` 变更（本次未引入新依赖）

- [x] **Step 4: 如有 lint 修复则提交**
