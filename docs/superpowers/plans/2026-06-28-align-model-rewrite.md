...
- [x] **2.3 实现 `realigning` 重入保护** — `realign(column)` 入口置 `this.realigning = true`，写完 `scrollTop`、本任务周期退出清 `false`。`onScroll` 收到 scroll 事件时先检 `realigning`，为 `true` 则直接 return 不派生 target。写重入保护测试：spy 派生函数；调一次 `realign` 后，模拟其触发的回流 scroll 事件进入 `onScroll`，断言派生函数未被调用、`getLockTarget()` 未变。
