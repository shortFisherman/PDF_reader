...
| 日期 | 编号 | 状态 | 提交 | 说明 |
|---|---|---|---|---|
| 2026-08-29 | P1-03 | 已完成 | `PENDING` | 术语合并改为模块级互斥锁 + 同目录临时文件 flush/fsync/close 后 `os.replace` 原子提交；写入/replace 失败保留旧 CSV 并清理临时文件；损坏或错误表头累计文件中止合并保留旧文件；新增 12 个术语表回归测试（含真实合并路径的迟到身份拒绝与并发不丢更新）。 |
