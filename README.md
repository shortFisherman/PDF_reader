...
- 启动失败不会一闪而过，不同阶段由不同界面说明：
  - **启动阶段失败**（便携目录不可写、端口被占用、发行包资源缺失、服务异常退出等）：弹出启动器错误窗口，
    说明原因并提供“复制诊断信息 / 打开日志目录 / 退出”；可恢复的错误还有对应入口（打开配置目录、
    打开程序目录，或“重试启动”重新走一遍启动流程）。
  - **配置问题**（缺少 API Key、字段非法、TOML 损坏等）：便携服务以受限配置页打开，由你在浏览器里修正并
    保存；保存后需重新启动，不会弹启动器窗口，也不会带错误配置继续运行。
  - **运行期模型或字体下载失败**：浏览器翻译状态区显示固定安全消息（服务端分类为稳定错误码
    `model_download_failed`），提示检查网络或代理后重试。上游原始异常正文既不会显示在页面上，
    也不会写入日志或诊断：服务端只记录分类后的稳定码（普通错误为 `translation_error`）。
- 错误窗口“复制诊断信息”只含应用版本、错误码、平台信息与脱敏后的日志细节，不含 API Key、Bearer token、
  控制或健康令牌、Prompt 与 PDF 正文，也不含带用户名的绝对路径；日志仍写在 `data/logs/`。
