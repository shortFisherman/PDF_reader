...
- Comet 项目级集成退役：官方 `comet uninstall . --scope project --force`（2026-09-08）
  完成卸载，工作树中的 `.agents/`、`.comet/`、`.opencode/` 与 `.codex/` Comet 安装物
  （技能、命令、规则与工作流守卫）全部移除；历史内容由 Git 保留追溯。
- 治理同步：工具目录治理文档不再把 `.agents/`、`.comet/` 列为在用/跟踪目录，清理
  `.gitignore` 与 lint 忽略中的 Comet 状态，并新增仓库治理回归，防止 Comet 项目级
  安装重新进入工作树。
