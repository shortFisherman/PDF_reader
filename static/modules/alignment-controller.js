export function createAlignmentController({ leftEl, rightEl }) {
    ...
    function realign(column) {
        ...
        // re-entering onScroll. Browsers coalesce scrollTop + scrollLeft
        // writes into a single scroll event, so one suppression flag suffices.
        // Without this, onScroll on the destination column would re-derive
        // currentTarget from its own scroll position and trigger a feedback
        // loop that causes persistent vertical misalignment at page boundaries.
    ...
    function onScroll(src) {
        // Suppress the scroll event that realign() generates on the
        // destination column. Without this, onScroll on the destination
        // would re-derive currentTarget from its own scroll position
        // and call realign back on the source column, creating a
        // feedback loop that causes persistent misalignment whenever
