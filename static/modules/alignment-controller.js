...
if (typeof window !== 'undefined' && window.__TEST_ALIGNMENT_CONTROLLER__) {
    if (typeof createAlignmentController !== 'function') {
        ...
        // Test (d): Re-entrance guard — realignTarget flag suppresses the
        // scroll event that realign() generates on the destination column.
        // This prevents the feedback loop: realign writes scrollTop →
        // browser fires scroll event → onScroll re-derives target →
        // calls realign on source column → misalignment at page boundaries.
        {
            const leftEl = document.createElement('div');
            const rightEl = document.createElement('div');
            Object.defineProperty(leftEl, 'clientHeight', { value: 500, configurable: true });
            ...
            // Set a lock target different from what onScroll would derive.
            // If the guard fails and onScroll runs during realign, it would
            // overwrite to {pageIndex:0, intraPageOffsetPx:0}.
            ctrl.setLockTarget(2, 100);
            const targetBefore = ctrl.getLockTarget();

            // realign sets realignTarget = rightEl, then writes scrollTop.
            // In a real browser, the subsequent scroll event on rightEl
            // reaches onScroll and is suppressed by src === realignTarget.
            ctrl.realign(rightEl);

            // Simulate the browser firing the scroll event: call onScroll
            // as the scroll event handler would. The guard should suppress it.
