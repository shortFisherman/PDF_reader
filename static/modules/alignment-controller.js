export function createAlignmentController({ leftEl, rightEl }) {
    ...
    var realignSuppressTarget = null;
    var realignSuppressCount = 0;
    ...
    function realign(column) {
        ...
        // Suppress the scroll events generated by the scrollTop/scrollLeft
        // writes below. Browsers may coalesce them into one event or fire
        // two separate events. The counter covers both cases.
        realignSuppressTarget = column;
        realignSuppressCount = 2;
    ...
    function onScroll(src) {
        // Suppress scroll events generated by realign() on the destination
        // column. Without this, onScroll would re-derive currentTarget from
        // the destination's scroll position and call realign back on the
        // source column — causing persistent vertical misalignment whenever
        // the viewport crosses a page boundary.
        if (src === realignSuppressTarget && realignSuppressCount > 0) {
            realignSuppressCount--;
            if (realignSuppressCount === 0) {
                realignSuppressTarget = null;
            }
            return;
        }
        // Scroll event from a different source — clear suppression.
        realignSuppressTarget = null;
        realignSuppressCount = 0;
